...
import {PoW} from "../typechain/index";
...
describe("Pow", () => {
    ...
    describe("mintToken", () => {
        ...
        it("should revert if not authorized", async () => {
            await expect(
                powContract
                    .connect(holder)
                    .mintToken(holder.address, "randomId")
            ).revertedWith("Not authorized");
            expect(
                await powContract.balanceOf(otherSigners[1].address)
            ).to.equal(0);
        });
    ...
    describe("setSaleIsActive", () => {
        ...
        it("should update saleActiveStatus", async () => {
            await powContract.setSaleIsActive(false);
            await expect(await powContract.saleIsActive()).to.be.false;
        });
        it("should revert if not admin", async () => {
            await expect(
