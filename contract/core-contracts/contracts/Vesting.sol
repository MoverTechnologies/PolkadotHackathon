    ...
    OwnableUpgradeable
{
    ...
    event AddVestingInfo(bytes32 indexed proofId, address indexed founder);

    event UpdateVestingInfo(bytes32 indexed proofId);

    event Released(bytes32 indexed proofId, address indexed mod);

    event Revoked(bytes32 indexed proofId, address indexed founder);
    ...
    modifier proofExists(bytes32 _proofId) {
        require(
            modInfoVesting[_proofId].founderAddress != address(0),
            "Proof doesn't exist"
        );
        _;
    }
     ...
     * Functions
     *************************************/
    function initialize(IERC20Upgradeable _depositedToken) public initializer {
        __Ownable_init();
    ...
    function addVestingInfo(
        ...
        require(
            depositedToken.balanceOf(_founderAddress) >= _amount,
            "insufficient token balance"
        );
        require(_jobEndTime > block.timestamp, "jobEndtime must be > now");
        // require(_duration != 0, "duration must be > 0");

        depositedToken.safeTransferFrom(
            _founderAddress,
            address(this),
            _amount
        );
     ...
     */
    function releaseAmount(bytes32 _proofId)
        public
        view
        virtual
        ...
        returns (uint256)
    {
        ModInfoVesting memory modVestingInfo = modInfoVesting[_proofId];
        if (block.timestamp < modVestingInfo.jobEndTime) {
            return 0;
        }

        uint256 remainingAmount = modVestingInfo.amount -
            modVestingInfo.released;

        if (
            block.timestamp >
            modVestingInfo.jobEndTime + modVestingInfo.duration
        ) {
            return remainingAmount;
        } else {
            return ((remainingAmount *
                (block.timestamp - modVestingInfo.jobEndTime)) /
                modVestingInfo.duration);
        }
    }
     ...
     */
    function setAgreementContractAddress(address _contractAddr)
        external
        onlyOwner
    {
